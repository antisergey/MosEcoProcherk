...
const _platform = (() => {
	...
	// ---------------------------------------------------------
	const notifyUser = (title, message, moreInfoMessage, iconUrl, moreInfoUrl) =>
	{
	...
	return {
		...
		injectedMediaQueriesOk() { return getApplicationName() !== 'Firefox'; },
	...
	args.forEach(currObj => {
		...
		for (const propName in currObj)
		{
			if (currObj.hasOwnProperty(propName))
			{
				...
				target[propName] = deep && propVal === Object(propVal) && typeof(propVal) !== 'function' ? JSON.parse(JSON.stringify(propVal)) : propVal;
			}
		}
	});
...
const _CONFIG = {
	...
	defaults: {
		...
		watchlist: [
			extend(true, { tz_id: 'Europe/London', name: _platform.getI18nMessage("location_London") }, tmp_defaults_watchlist_item_settings),
			extend(true, { tz_id: 'America/New_York', name: _platform.getI18nMessage("location_New_York") }, tmp_defaults_watchlist_item_settings),
			extend(true, { tz_id: 'Asia/Hong_Kong', name: _platform.getI18nMessage("location_Hong_Kong") }, tmp_defaults_watchlist_item_settings)
		]
	},
...
const getMapUrl = (item, itemZone) => {
	...
	const getGoogleMapsUrl = (query, latitude, longitude) =>
	{
		...
		if (latitude !== null && longitude !== null)
			url += `&ll=${Math.round(latitude * 1e+5) / 1e+5},${Math.round(longitude * 1e+5) / 1e+5}`;
	...
	if (item.hasOwnProperty('lat_decimal') && item.hasOwnProperty('long_decimal')) // legacy support - properties no longer generated on adding to watchlist
		mapUrl = getGoogleMapsUrl(item.name, item.lat_decimal, item.long_decimal);
	else if (typeof(item.coords) === 'object')
		mapUrl = getGoogleMapsUrl(item.name, item.coords.lat_decimal, item.coords.long_decimal);
	else if (item.coords === 'na') { // not available - search
	...
	else if (item.coords !== "none" && typeof(itemZone.coords) === 'object') // i.e. default value of coords is essentially 'follow_zone'
		mapUrl = getGoogleMapsUrl(item.name, itemZone.coords.lat_decimal, itemZone.coords.long_decimal);
...
const setStorage = (data, storageArea) =>
{
	...
	return Promise.all(asyncEvents).then(() => extend(true, tmpStorage.local, tmpStorage.sync));
};
...
const getOrRemoveStorage = (getOrRemove, nameArray, storageArea) =>
{
	...
	.then(arr => {
		...
		{
			if (arr[i] !== null)
				retObj = extend(true, retObj, arr[i]);
		}
...
// ---------------------------------------------------------
const getStorage = (nameArray, storageArea) => getOrRemoveStorage('get', nameArray, storageArea);
const removeStorage = (nameArray, storageArea) => getOrRemoveStorage('remove', nameArray, storageArea);
...
const getZonesForWatchlist = (watchlist, watchlistZones) =>
{
	...
	{
		return getStorage(['tz_db'])
		.then(storageItems => {
			const zoneIds = Object.keys(missingZones);

			for (const zoneId of zoneIds)
			{
				newWatchlistZones[zoneId] = extend(true, {}, storageItems.tz_db.zones[zoneId]);
			}
...
window.foxclocks.utils = /* export default */ extend({}, _platform, { // MODULE: restore export 
